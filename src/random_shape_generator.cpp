...
RandomShapeGenerator::RandomShapeGenerator( GridCollisionChecker2D* cc, int minNumSides, int maxNumSides, double minRadius, double maxRadius) : m_cc(cc) {
...
float getRandomFloat(float min, float max) {
    float random = rand();
    return min + static_cast<float> (random) / (static_cast <float>
        (RAND_MAX/(max-min)));
}
...
std::vector< std::vector<int> > toGrid (std::vector< Vector3d > points, sbpl::OccupancyGrid& grid) {
...
std::vector< Vector3d > toWorld (std::vector< std::vector<int> > points, sbpl::OccupancyGrid& grid) {
...
std::vector<Shape> RandomShapeGenerator::generateRandomShapes( int num, sbpl::OccupancyGrid& grid, int seed) {
    srand(seed);
    ...
    for (int i = 0; i < num; i++) {
        ...
        for(int j = 0;j < n_sides; j++) {
            ...
            ROS_INFO("%f", theta);
            vertices.push_back( Vector3d { radius*cos(theta), radius*sin(theta), 0 } );
        ...
        for (int j = 0; j < n_sides; j++) {
            ...
            m_cc->interpolatePath( toRobotState( vertices[j] ), toRobotState( vertices[(j+1)%n_sides] ), edge_);
...
// of pixels.
std::vector <std::vector<int> > getInflatedWindow (std::vector<int> point, int inflationRadius=1) {

    std::vector< std::vector<int> > neighbours;

    for(int i = point[0] - inflationRadius; i <= point[0] + inflationRadius; i++) {
        for(int j = point[1] - inflationRadius; j <= point[1] + inflationRadius; j++) {
            //if (i == j )
                //continue;
            std::vector<int> currPoint = { i, j, point[2] };
            // Check if it is already part of the shape.
            //if ( find( points.begin(), points.end(), currPoint ) != points.end() )
...
Shape RandomShapeGenerator::inflateShape(Shape shape, sbpl::OccupancyGrid& grid, int inflationRadius=1) {
    std::vector< std::vector<int> > boundaryGridPoints = toGrid(shape.getBoundary(), grid);
    // Set insertions is log n.
    std::set< std::vector<int> > inflatedGridPoints;

    for (auto point : boundaryGridPoints) {
        std::vector< std::vector<int> > neighbours = getInflatedWindow(point, inflationRadius);
        for ( auto ele : neighbours )
            inflatedGridPoints.insert( ele );
    }
    std::vector< std::vector<int> > inflatedGridPointsV (inflatedGridPoints.begin(), inflatedGridPoints.end());
